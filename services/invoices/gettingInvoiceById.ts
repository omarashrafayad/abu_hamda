...
function useGettingInvoiceById() {
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [invoice, setInvoice] = useState<any>(null);

  const gettingInvoiceById = useCallback(async (id: number | string) => {
    if (!id) return;
    setLoading(true);
    setError(null);
    const timestamp = new Date().getTime();
    await AxiosInstance.get(`/api/Invoices/${id}?t=${timestamp}`)
      .then((res) => {
...
export default useGettingInvoiceById;
