...
import useGetSpecialOffers from "@/services/specialOffers/getAllSpecialOffers";
...
const SpecialOffersTable = () => {
    ...
    const { offers: data, loading, getAllSpecialOffers } = useGetSpecialOffers();
    const t = useTranslations("SpecialOffers");
    const columns = baseColumns({ refresh: getAllSpecialOffers, t });

    const [filteredOffers, setFilteredOffers] = useState<SpecialOfferType[]>([]);
        ...
        <div className="w-full">
            <div className="flex flex-wrap justify-between items-center py-4 px-6 border-b border-solid border-default-200">
                <div className="flex-1 max-w-sm">
                    <SearchInput data={data ?? []} setFilteredData={setFilteredOffers} filterKey={"id"} placeholder={t("searchPlaceholder") || "Search by ID..."} />
                </div>
                <div className="flex items-center gap-4 flex-wrap">
                    <Link href="/dashboard/add-special-offer">
