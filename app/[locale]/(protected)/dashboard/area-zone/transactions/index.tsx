...
const AreaZonesTable = () => {
    const [sorting, setSorting] = React.useState<SortingState>([]);
    const [columnFilters, setColumnFilters] = React.useState<ColumnFiltersState>([]);
    const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({});
    const [rowSelection, setRowSelection] = React.useState({});

    const { areaZones: data, loading, getAllAreaZones } = useGetAreaZones();
    const t = useTranslations("area_zones");
    const columns = baseColumns({ refresh: getAllAreaZones, t });

    const [filteredAreaZones, setFilteredAreaZones] = useState<AreaZoneType[]>([]);

    const table = useReactTable({
        data: filteredAreaZones ?? [],
        columns,
    ...
    return (
        <div className="w-full">
            <div className="flex flex-wrap justify-end items-center py-4 px-6 border-b border-solid border-default-200 gap-4">
                <SearchInput data={data ?? []} setFilteredData={setFilteredAreaZones} filterKey={"areaName"} placeholder="Search area zones..." />
                <div className="#flex-none">
                    <div className="flex items-center gap-4 flex-wrap">
                        <Link href="/dashboard/add-area-zone">
