...
import useGetStockProducts from "@/services/stockProducts/getAllStockProducts";
...
const StockProductsTable = () => {
    ...
    const { stockProducts: data, loading, getAllStockProducts } = useGetStockProducts();
    const t = useTranslations("stockProducts");
    const columns = baseColumns({ refresh: getAllStockProducts, t });

    const [filteredStockProducts, setFilteredStockProducts] = useState<StockProductType[]>([]);
        ...
        <div className="w-full">
            <div className="flex flex-col sm:flex-row justify-between items-center py-4 px-6 border-b border-solid border-default-200 gap-4">
                <div className="w-full sm:w-auto flex-1">
                    <SearchInput data={data ?? []} setFilteredData={setFilteredStockProducts} filterKey={"productName"} placeholder={t("searchPlaceholder")} />
                </div>
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/add-stock-product">
