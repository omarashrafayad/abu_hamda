...
import useDeleteCity from "@/services/cities/deleteCity";

export const baseColumns = ({ refresh, t }: { refresh: () => void; t: (key: string) => string }): ColumnDef<CityType>[] => [
    {
        accessorKey: "name",
        header: t("city_name"),
    ...
    },
    {
        ...
        cell: ({ row }) => <span className="font-medium text-default-900">{row.getValue("countryName") || t("unknown")}</span>,
    },
    {
        id: "actions",
        accessorKey: "action",
        ...
        cell: ({ row }) => {
            ...
            const { deleteCity, loading } = useDeleteCity();

            const handleDelete = () => {
                const toastId = toast(t("delete_city"), {
                    description: t("delete_city_confirm"),
                    action: (
                        <div className="flex justify-end mx-auto items-center my-auto gap-2">
                            ...
                            <Button
                                ...
                                onClick={async () => {
                                    try {
                                        const isSuccess = await deleteCity(id as string);
                                        
                                        toast.dismiss(toastId);

                                        if (isSuccess) {
                                            toast.success(t("city_deleted"), {
                                                description: t("city_deleted_success"),
                                            });
                                            refresh(); 
                                        } else {
